...
export default function GenericTable<T>({ data, columns, meta, onPageChange, isLoading, onRowClick }: GenericTableProps<T>) {
  return (
      ...
      {meta && meta.totalPages > 1 && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <button className="btn btn-ghost" style={{ padding: '0.25rem' }} disabled={meta.page <= 1} onClick={() => onPageChange && onPageChange(meta.page - 1)}>
              <ChevronLeft size={20} />
            </button>
            <div style={{ fontSize: '0.875rem', fontWeight: 500, padding: '0 0.5rem' }}>
              Page {meta.page} of {meta.totalPages}
            </div>
            <button className="btn btn-ghost" style={{ padding: '0.25rem' }} disabled={meta.page >= meta.totalPages} onClick={() => onPageChange && onPageChange(meta.page + 1)}>
