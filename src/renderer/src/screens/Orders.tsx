...
export default function Orders() {
  ...
  const { data, isLoading, isError, error } = useOrders(page, limit, filterType === 'all' ? undefined : filterType)
          ...
          return (
            ...
            >
              {statusOptions.map((option) => {
                return (
                  <option key={option.value} value={option.value} style={{ color: 'var(--text-primary)' }}>
                    {option.label}
                  </option>
                )
              })}
  ...
  const filteredOrders = (data?.data || []).filter((order) => {
    ...
    return order.id?.toLowerCase().includes(query) || order.type?.toLowerCase().includes(query) || order.status?.toLowerCase().includes(query)
  })
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          ...
          <div className="segment-control">
            <button className={`segment-item ${filterType === 'all' ? 'active' : ''}`} onClick={() => handleFilterChange('all')}>
              All
            </button>
            <button className={`segment-item ${filterType === OrderType.Sales ? 'active' : ''}`} onClick={() => handleFilterChange(OrderType.Sales)}>
              Sales
            </button>
            <button className={`segment-item ${filterType === OrderType.Purchase ? 'active' : ''}`} onClick={() => handleFilterChange(OrderType.Purchase)}>
              Purchase
            </button>
          </div>
      ...
      {isError ? <div style={{ color: 'var(--accent-danger)' }}>Failed to load orders: {(error as Error)?.message}</div> : <GenericTable data={filteredOrders} columns={columns} meta={data?.meta || { page, limit, total: 0, totalPages: 1 }} onPageChange={setPage} isLoading={isLoading} onRowClick={(item) => navigate(`/orders/${item.id}`)} />}
