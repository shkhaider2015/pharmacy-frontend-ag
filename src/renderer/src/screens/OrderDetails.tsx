...
export default function OrderDetails() {
  ...
  if (isError || !order) {
    return <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--accent-danger)' }}>Failed to load order details: {(error as Error)?.message || 'Order not found'}</div>
  }
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button className="btn btn-ghost" onClick={() => navigate('/orders')} style={{ padding: '0.5rem', borderRadius: '50%' }}>
            <ArrowLeft size={20} />
          </button>
          <div>
            ...
            <div style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginTop: '0.25rem' }}>Created on {new Date(order.createdAt || 0).toLocaleString()}</div>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '1rem 1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--accent-primary)' }}>${order.totalAmount || '0.00'}</div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <GenericTable data={order.items || []} columns={columns} isLoading={false} />
      </div>
    </div>
  )
}
