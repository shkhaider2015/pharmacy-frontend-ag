...
export default function Inventory() {
  ...
  const filteredInventory = (data?.data || []).filter((item) => {
    ...
    return item.productName?.toLowerCase().includes(query) || item.batchNumber?.toLowerCase().includes(query)
  })
  ...
  return (
    <div className="animate-fade-in">
      ...
      {isError ? <div style={{ color: 'var(--accent-danger)' }}>Failed to load inventory: {(error as Error)?.message}</div> : <GenericTable data={filteredInventory} columns={columns} meta={data?.meta || { page, limit, total: 0, totalPages: 1 }} onPageChange={setPage} isLoading={isLoading} />}

      <Modal isOpen={isFormOpen} onClose={handleCloseForm} title={editingId ? 'Edit Inventory Batch' : 'Add Inventory Batch'}>
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'flex', gap: '1rem' }}>
            <div style={{ flex: 2 }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                Product <span style={{ color: 'var(--accent-danger)' }}>*</span>
              </label>
              <select
                required
                className="input-field"
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                Manufacturing Date <span style={{ color: 'var(--accent-danger)' }}>*</span>
              </label>
              <input
                type="date"
                required
            ...
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem' }}>
                Expiry Date <span style={{ color: 'var(--accent-danger)' }}>*</span>
              </label>
              <input
                type="date"
                required
